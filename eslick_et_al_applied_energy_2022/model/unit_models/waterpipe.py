...
##############################################################################
"""
Pipe model for water or steam flow
main equations:
...
see property package for documentation.}"""))
    CONFIG.declare("contraction_expansion_at_end",ConfigValue(
        ...
        doc='define if pressure drop due to contraction or expansion needs to be considered'))
    ...
    def _set_geometry(self):
        ...
        if not self.config.contraction_expansion_at_end is "None":
            self.area_ratio = Var(
                initialize=1,
                doc="cross section area ratio of exit end to pipe")

        # Volume constraint
        @self.Constraint(self.flowsheet().config.time, doc="Total volume of all pipes")
        def volume_eqn(b, t):
            return b.volume[t] == 0.25*b.pi*b.diameter**2*b.length*b.count
    ...
    def _make_performance(self):
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Vecolity of fluid inside pipe")
        ...
        def pressure_change_friction_eqn(b, t):
            return b.deltaP_friction[t] * b.diameter == \
                   -0.5 * b.control_volume.properties_in[t].dens_mass_phase[phase] * \
        ...
        def pressure_change_gravity_eqn(b, t):
            return b.deltaP_gravity[t] == -b.gravity * \
                   b.control_volume.properties_in[t].dens_mass_phase[phase] * b.elevation_change
        ...
        def pressure_change_area_change_eqn(b, t):
            if self.config.contraction_expansion_at_end == "contraction":
                # contraction stagnant pressure drop K*0.5*den*v2^2 plus acceleration drop
                # -(K+1)(0.5*den*v2^2) + (0.5*den*v1^2), K is loss coefficient by a polynormial expression
                # area_ratio < 1
                return b.deltaP_area_change[t] == -(0.1602*b.area_ratio**2 - 0.646*b.area_ratio + 1.4858)* \
                0.5*b.control_volume.properties_out[t].dens_mass_phase[phase]*(b.velocity[t]/b.area_ratio)**2 + \
                0.5*b.control_volume.properties_out[t].dens_mass_phase[phase]*b.velocity[t]**2
            elif self.config.contraction_expansion_at_end == "expansion":
                # den*v^2*(area_ratio-1)/area_ratio^2
                # area_ratio > 1
                return b.deltaP_area_change[t] == b.control_volume.properties_out[t].dens_mass_phase[phase]*\
        ...
        def pressure_change_total_eqn(b, t):
            return b.deltaP[t] == (b.deltaP_friction[t] + b.deltaP_gravity[t] + b.deltaP_area_change[t])
