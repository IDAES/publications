...
from pyomo.environ import SolverFactory, TransformationFactory, Var, value, Param, asin, cos
...
from idaes.power_generation.unit_models.helm import MomentumMixingType
from idaes.core.util import from_json, to_json, StoreSpec
from idaes.core.util.model_statistics import degrees_of_freedom
from idaes.core import useDefault
from idaes.core.util.config import is_physical_parameter_block
import idaes.logger as idaeslog
#from idaes.power_generation.unit_models.helm import HelmNtuCondenser as Condenser
#from idaes.power_generation.unit_models.helm import HelmNtuCondenserData as CondenserData
from unit_models import HelmNtuCondenser as Condenser
from unit_models import HelmNtuCondenserData as CondenserData
...
class FWHCondensing0DData(CondenserData):
    def build(self):
        ...
        self.vol_frac_shell = Var(initialize=0.7, doc="volume fraction f shell side")
        self.id_shell = Var(initialize=1, doc="shell inside diameter")
        self.length_tube = Var(initialize=10, doc="length of straint tube for the condenser section")
        self.level = Var(self.flowsheet().config.time, initialize=1, doc="water level")

        # Expressure for the angle from the drum center to the circumference point at water level
        @self.Expression(doc="angle of water level")
        def r_shell(b):
            return b.id_shell/2

        # Expressure for the angle from the drum center to the circumference point at water level (-pi/2 and pi/2)
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Calculate the volume of shell side based on water level")
        def shell_volume_eqn(b, t):
            return b.shell.volume[t] == ((b.alpha[t]+0.5*b.pi)*b.r_shell**2 + \
                   b.r_shell*cos(b.alpha[t])*(b.level[t]-b.r_shell))*b.length_tube*b.vol_frac_shell
        ...
        def pressure_change_total_eqn(b, t):
            return b.shell.deltaP[t] == b.gravity*b.level[t]*b.shell.properties_out[t].dens_mass_phase["Liq"]
    ...
    def initialize(self, *args, **kwargs):
        ...
        # fix volume and pressure drop since the condenser initialization dosen't require them
    ...
    def build(self):
        ...
        if config.has_drain_mixer:
            mix_cfg = {  # general unit model config
                "dynamic": False, #config.dynamic,  revised by JM, always set as steady-state
                "has_holdup": False, #config.has_holdup, revised by JM
