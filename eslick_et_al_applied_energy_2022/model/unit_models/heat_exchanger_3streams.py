...
side 1 is hot stream, side 2 and 3 is cold streams
"""
...
**False** - exclude pressure change terms.}"""))
    ...
    def _make_performance(self):
        ...
        self.temperature_driving_force_side_2 = Var(self.flowsheet().config.time,
                                             initialize=1.0,
                                             doc='Mean driving force for heat exchange')

        # Temperature driving force
        self.temperature_driving_force_side_3 = Var(self.flowsheet().config.time,
                                             initialize=1.0,
                                             doc='Mean driving force for heat exchange')
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Log mean temperature difference calculation")
        def LMTD_side_2(b, t):
            return b.temperature_driving_force_side_2[t] == ((b.side_2_inlet_dT[t]**0.3241 +
                   b.side_2_outlet_dT[t]**0.3241)/1.99996)**(1/0.3241)

        # Driving force side 3
        @self.Constraint(self.flowsheet().config.time, doc="Log mean temperature difference calculation")
        def LMTD_side_3(b, t):
            return b.temperature_driving_force_side_3[t] == ((b.side_3_inlet_dT[t]**0.3241 +
                   b.side_3_outlet_dT[t]**0.3241)/1.99996)**(1/0.3241)

        # Heat duty side 2
        @self.Constraint(self.flowsheet().config.time, doc="Heat transfer rate")
        def heat_duty_side_2_eqn(b, t):
            return b.heat_duty_side_2[t] == (b.ua_side_2[t]*b.temperature_driving_force_side_2[t])

        # Heat duty side 3
        @self.Constraint(self.flowsheet().config.time, doc="Heat transfer rate")
        def heat_duty_side_3_eqn(b, t):
            return b.heat_duty_side_3[t] == (b.ua_side_3[t]*b.temperature_driving_force_side_3[t])

        # Energy balance equation
        @self.Constraint(self.flowsheet().config.time, doc="Energy balance between two sides")
        def heat_duty_side_1_eqn(b, t):
            return -b.heat_duty_side_1[t]*(1-b.frac_heatloss) == (b.heat_duty_side_2[t] + b.heat_duty_side_3[t])
    ...
    def _make_con_current_side_2(self):
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Side 2 inlet temperature difference")
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Side 2 outlet temperature difference")
    ...
    def _make_con_current_side_3(self):
        ...
        """
        # Temperature Differences
        @self.Constraint(self.flowsheet().config.time, doc="Side 3 inlet temperature difference")
        def side_3_inlet_dT_eqn(b, t):
            return b.side_3_inlet_dT[t] == (
                       b.side_1.properties_in[t].temperature -
                       b.side_3.properties_in[t].temperature)

        @self.Constraint(self.flowsheet().config.time, doc="Side 3 outlet temperature difference")
    ...
    def initialize(blk, state_args_1=None, state_args_2=None, state_args_3=None,
                   outlvl=4, solver='ipopt', optarg={'tol': 1e-6}):
        '''
