...
##############################################################################
"""
PID controller block, Revised to use pyomo.DAE to calculate integral and differential parts
"""

__author__ = "John Eslick and Jinliang Ma"


from pyomo.environ import Var, Param, Constraint, Expression, Reference, exp, log
...
@declare_process_block_class("PIDController", doc=
    """This is a PID controller block. Based on UnitModelBlockData to configure the dynamic flag based on parent""")
...
Default is PI"""))
    ...
    def build(self):
        ...
        if self.config.dynamic==True:
            ...
            # Parameters
            self.smooth_eps = Param(mutable=True, initialize=1e-4, doc="Smoothing parameter for controller output limits")
            self.mv_lb = Param(mutable=True, initialize=0.05, doc="controller output lower bound")  #not use 0 for valve since it could cause negative flow
            self.mv_ub = Param(mutable=True, initialize=1, doc="controller output upper bound")

            # Variable for basic controller settings may change with time.
            self.setpoint = Var(time_set, initialize=0.5, doc="Setpoint")
            self.gain_p = Var(time_set, initialize=0.1, doc="Gain for proportional part")
            if self.config.type == 'PI' or self.config.type == 'PID':
                self.gain_i = Var(time_set, initialize=0.1, doc="Gain for integral part")
            if self.config.type == 'PD' or self.config.type == 'PID':
                self.gain_d = Var(time_set, initialize=0.01, doc="Gain for derivative part")
            ...
            if self.config.type == 'PI' or self.config.type == 'PID':
                self.integral_of_error = Var(time_set, initialize=0, doc="Integral term")
                self.error_from_integral = DerivativeVar(self.integral_of_error, wrt = self.flowsheet().config.time, initialize=0)
            ...
            if self.config.type == 'PID' or self.config.type == 'PD':
                self.derivative_of_error = DerivativeVar(self.error, wrt = self.flowsheet().config.time, initialize=0)
            ...
            def mv_unbounded(b, t):
                if self.config.type == 'PID':
                    return b.mv_ref + b.gain_p[t]*b.error[t] + b.gain_i[t]*b.integral_of_error[t] + b.gain_d[t]*b.derivative_of_error[t]
                elif self.config.type == 'PI':
                    return b.mv_ref + b.gain_p[t]*b.error[t] + b.gain_i[t]*b.integral_of_error[t]
                elif self.config.type == 'PD':
                    return b.mv_ref + b.gain_p[t]*b.error[t] + b.gain_d[t]*b.derivative_of_error[t]
                else:
            ...
            def mv_eqn(b,t):
                if t==b.flowsheet().config.time.first():
                    ...
                    if self.config.bounded_output==True:
                        ...
                        return (b.mv[t]-b.mv_lb)*(1+exp(-4/(b.mv_ub-b.mv_lb)*(b.mv_unbounded[t]-(b.mv_lb+b.mv_ub)/2))) == b.mv_ub-b.mv_lb
                    else:
                        return b.mv[t] == b.mv_unbounded[t]

            @self.Expression(time_set, doc="integral error at error 0 and mv_ref")
            def integral_of_error_ref(b, t):
                return ((b.mv_lb+b.mv_ub)/2-b.mv_ref-log((b.mv_ub-b.mv_lb)/(b.mv_ref-b.mv_lb)-1)/4*(b.mv_ub-b.mv_lb))/b.gain_i[t]
