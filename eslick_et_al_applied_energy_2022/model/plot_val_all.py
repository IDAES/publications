...
pretty_labels = {
    ...
    "S834": ("Throttle Steam Temperature (K)", 1),
    "S839C": ("Throttle Steam Pressure (kPa)", 1e-3),
...
if __name__ == "__main__":
    df = pd.read_csv("results/validate_allx.csv")
    ...
    for col in df.columns:
        ...
        # skip anything I don't want to show
        if tag not in pretty_labels:
            continue
    ...
    for pdf in plots:
        writer.append(pdf)
    writer.write("val_plots_all.pdf")

    with open("val_nrmse_all.csv", "w", newline="") as f:
        w = csv.writer(f)
        for row in rows:
            w.writerow(row)
