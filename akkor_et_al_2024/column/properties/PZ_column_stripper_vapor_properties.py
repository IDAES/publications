...
"""
Vapor phase properties for the stripper column, mixture of CO2-H2O-PZ
"""
...
def _add_flow_mol_comp(self):
    y_in = {"CO2": 0.405, "H2O": 0.589, "PZ": 0.006}
...
def _add_density(self):
    ...
    if self.config.defined_state is False:
        ...
        def dens_rule(b):
            return self.dens_mass * 8.314 * self.temperature == sum(
                self.pressure * self.mole_frac_comp[j] * self.mw_comp[j]
...
def _add_diffusivity(self):
    ...
    if self.config.defined_state is False:
        ...
        def diff_rule(b, j):
            # p = pyunits.convert(b.pressure, to_units=pyunits.atm)
            ...
            return (
                ...
                * (
                    ...
                    / 1000
                    * 0.00987
                    * (self.diff_vol[j] ** (1.0 / 3.0) + 20.1 ** (1.0 / 3.0)) ** 2
                )
                == 10 ** (-7)
                * (self.temperature**1.75)
                * (1 / (self.mw_comp[j] * 1000) + 1 / 29) ** 0.5
            )
...
def _add_concentration_tot(self):
    ...
    def concentration_tot_rule(b):
        return self.pressure == self.conc_mol * 8.314 * self.temperature
...
def _add_pressure_comp(self):
    ...
    if self.config.defined_state is False:
        ...
        def pressure_comp_rule(b, j):
            return self.pressure_comp[j] == self.pressure * self.mole_frac_comp[j]
...
def _add_heat_capacity_comp(self):
    ...
    def heat_capacity_comp_rule(b, j):
        return (
            self.cp_mol_comp[j]
            == self.A[j]
            + self.B[j] * self.temperature
            + self.C[j] * self.temperature**2
            + self.D[j] * self.temperature**3
        )

    self.heat_capacity_comp_eq = Constraint(
        ["CO2", "H2O"], rule=heat_capacity_comp_rule
    )
...
def _add_cp_vol(self):
    ...
    def heat_capacity_rule(b):
        return self.cp_vol == sum(
            self.cp_mol_comp[j] * self.conc_mol_comp[j] for j in ["CO2", "H2O"]
        )
...
def _add_viscosity(self):
    ...
    if self.config.defined_state is False:
        ...
        def viscosity_comp_rule(b, j):
            return (
                self.viscosity_comp[j]
                == (10 ** (self.A_vis[j] * (1 / self.temperature - 1 / self.B_vis[j])))
                * 0.01
            )
            ...
            if j == 1:
                ...
                return (
                    ...
                    * (
                        ...
                        + cst_co2_h2o_regen * self.mole_frac_comp["H2O"]
                    )
                ...
                return (
                    ...
                    * (
                        ...
                        + cst_h2o_co2_regen * self.mole_frac_comp["CO2"]
                    )
                    == self.viscosity_comp["H2O"] * self.mole_frac_comp["H2O"]
                )

        self.mu_intermediate_eq = Constraint(
            [1, 2], rule=mu_intermediate_rule, doc="Wilke (1950)"
        )
    ...
    if self.config.defined_state is False:
        ...
        def viscosity_rule(m):
            return (
                self.visc_d == (self.mu_intermediate[1] + self.mu_intermediate[2]) * 100
            )
