...
def _add_equilibrium_pressure(self):
    ...
    if self.config.defined_state is False:
        ...
        def equilibrium_pressure_rule(b, j):
            if j == "CO2":
                return self.equilibrium_pressure[j] / pyunits.Pa == exp(
                ...
                return self.equilibrium_pressure[j] / pyunits.Pa == self.mole_frac_comp[
                    j
                ] * exp(
        ...
        self.equilibrium_pressure_eq = Constraint(
            ["CO2", "H2O", "PZ"], rule=equilibrium_pressure_rule
        )
...
def _add_diffusivity_cst(self):
    ...
    self.diffusivity_param_1 = Param(
        initialize=0.024 * 10 ** (-4), mutable=True, units=pyunits.m**2 / pyunits.s
    )
...
def _add_saturation_pressure(self):
    self.P_sat = Var(
        ["CO2", "H2O", "PZ"], initialize=500000, bounds=(0, None), units=pyunits.Pa
    )
    ...
    self.saturation_pressure_eq = Constraint(
        ["CO2", "H2O", "PZ"], rule=saturation_pressure_rule
    )
