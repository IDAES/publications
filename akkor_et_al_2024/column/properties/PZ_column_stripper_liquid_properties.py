...
def _add_loading(self):
    self.alpha = Var(
        initialize=0.35,
        units=pyunits.dimensionless / pyunits.dimensionless,
        doc="Solvent loading",
    )
...
def _add_equilibrium_pressure(self):
    ...
    if self.config.defined_state is False:
        ...
        def equilibrium_pressure_rule(b, j):
            if j == "CO2":
                return self.equilibrium_pressure[j] == exp(
                    35.3
                    - 11054 / self.temperature
                    - 18.9 * (self.alpha**2)
                    + 4958 * self.alpha / self.temperature
                    + 10163 * (self.alpha**2) / self.temperature
                )
            elif j == "H2O":
                return self.equilibrium_pressure[j] == self.mole_frac_comp[j] * exp(
                    72.55
                    - 7206.7 / self.temperature
                    - 7.1385 * log(self.temperature)
                    + 4.04 * 10 ** (-6) * (self.temperature**2)
                )
            elif j == "PZ":
                return self.equilibrium_pressure[j] == self.mole_frac_comp[j] * exp(
                    172.78
                    - 13492 / self.temperature
                    - 21.91 * log(self.temperature)
                    + 1.378 * 10 ** (-5) * (self.temperature**2)
                )
...
def _add_density(self):
    ...
    if self.config.defined_state is False:
        ...
        def dens_H2O_rule(b):
            return (
                self.dens_mass_H2O
                == 1002.3
                - 0.1321 * (self.temperature - 273.15)
                - 0.00308 * (self.temperature - 273.15) ** 2
            )
    ...
    if self.config.defined_state is False:
        ...
        def dens_rule(b):
            return self.dens_mass == self.dens_mass_H2O * (
                0.0407 * self.mole_frac_comp["CO2"] / self.mw_comp["CO2"]
                + 0.008 * self.mole_frac_comp["PZ"] / self.mw_comp["PZ"]
                + 0.991
            )
...
def _add_viscosity_water(self):
    self.viscosity_water = Var(initialize=0.005, bounds=(0, None), units=pyunits.cp)

    if self.config.defined_state is False:

        def viscosity_water_rule(b):
            return self.viscosity_water == (10 ** (-3)) * 1.002 * exp(
                2.303
                * (
                    1.3272 * (293 - self.temperature)
                    - 0.001053 * (self.temperature - 293) ** 2
                )
                / (self.temperature - 168)
            )
...
def _add_viscosity(self):
    ...
    if self.config.defined_state is False:
        ...
        def viscosity_rule(b):
            return self.visc_d == self.viscosity_water * exp(
                (
                    (487.52 * self.weight_percent_PZ + 1389.31) * self.temperature
                    + (1.58 * self.weight_percent_PZ + 4.5)
                )
                * (
                    self.alpha
                    * (8.73 * self.weight_percent_PZ - 0.0038 * self.temperature - 0.3)
                    + 1
                )
                * self.weight_percent_PZ
                / (self.temperature**2)
...
def _add_rxn_rate_cst(self):
    ...
    if self.config.defined_state is False:
        ...
        def rxn_rate_cst_rule(b):
            return self.reaction_rate == 5.37 * 10 * exp(
                (-3.36 * 10**4) * (1 / self.temperature - 1 / 298.15) / 8.314
            )
...
def _add_henrys_cst(self):
    ...
    if self.config.defined_state is False:
        ...
        def henry_cst_rule(b):
            return (
                self.henry
                == 1.7107 * 10**7 * exp(-1886.1 / self.temperature) * 0.0001
            )
...
def _add_diffusivity_cst(self):
    ...
    if self.config.defined_state is False:
        ...
        def diffusivity_cst_rule(b):
            return self.diffusivity_cst == 0.024 * exp(-2122 / self.temperature) * (
                10 ** (-4)
        ...
        def equilibrium_cst_rule(b, j):
            if j == 1 or j == 2 or j == 3:
                return self.log_k_eq[j] == self.param_A[j] + self.param_B[
                    j
                ] / self.temperature + self.param_C[j] * log(self.temperature)
            else:
                return (
                    self.log_k_eq[j]
...
def _add_concentration_spec(self):
    ...
    if self.config.defined_state is False:
        ...
        def ln_concentration_spec_rule(b, j):
            return self.ln_concentration_spec[j] == log(self.concentration_spec[j])
...
def _add_integral_heat_capacity(self):
    ...
    def integrated_heat_capacities_rule(f, j):
        if j == "CO2":
            ...
            return f.Cp_int[j] == 237 * (f.temperature - 413)
