...
def economic_model(m, num_segments):
    # Costing correlations for purchased equipments are from Warren D. Seider et al.
    ...
    m.base_cost_index = Param(initialize=567, doc="Seider et al., costs from 2013")
    ...
    m.D_flash_tank = Expression(expr=m.afs.stripper.diameter * 39.37, doc="inch")

    m.L_flash_tank = Expression(
        expr=4 * m.flash_tank_volume * 61023.7 / np.pi / m.D_flash_tank**2,
    )
    ...
    m.pump_1_electric_motor_cost = Expression(
        expr=m.pump_ref_electric_motor_cost * (m.pump_1_P_c / m.pump_ref_P_c) ** 0.6
    )
    ...
    m.pump_2_electric_motor_cost = Expression(
        expr=m.pump_ref_electric_motor_cost * (m.pump_2_P_c / m.pump_ref_P_c) ** 0.6
    )
    ...
    # m.blower_fan_material_factor = Param(
    #     initialize=0.6, doc="cast aliminum blades (Seider)"
    # )
    m.blowers_bare_cost = Expression(
        expr=2
        # * m.blower_fan_material_factor
        * m.blower_ref_cost
    ...
    m.steam_oc_calculation = Constraint(
        expr=m.steam_oc
        == m.steam_flowrate * m.h_steam / 1e9 * m.operation_time * m.steam_price
    )
