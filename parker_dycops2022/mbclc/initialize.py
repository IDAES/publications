...
def set_default_inlet_conditions(m):
    ...
    dynamic_inputs = {
        ...
        "fs.MB.gas_inlet.pressure[*]": 2.00,
        "fs.MB.gas_inlet.temperature[*]": 298.15,
        "fs.MB.gas_inlet.mole_frac_comp[*,CO2]": 0.02499,
        "fs.MB.gas_inlet.mole_frac_comp[*,H2O]": 0.00001,
        "fs.MB.gas_inlet.mole_frac_comp[*,CH4]": 0.97500,

        "fs.MB.solid_inlet.flow_mass[*]": 591.4,
        "fs.MB.solid_inlet.temperature[*]": 1183.15,
        "fs.MB.solid_inlet.particle_porosity[*]": 0.27,
        "fs.MB.solid_inlet.mass_frac_comp[*,Fe2O3]": 0.45,
        "fs.MB.solid_inlet.mass_frac_comp[*,Fe3O4]": 1e-9,
        "fs.MB.solid_inlet.mass_frac_comp[*,Al2O3]": 0.55,
        }
...
def set_optimal_design_vars(m):
    ...
    """
    design_vars = {
            "fs.MB.bed_diameter": 4.58,
            "fs.MB.bed_height": 4.60,
            }
    design_vars = fix_design_variables(m, design_vars)
    return design_vars
...
def set_optimal_inlet_conditions(m):
    ...
    time = m.fs.time
    dynamic_inputs = {
            "fs.MB.gas_inlet.flow_mol[*]": 128.0,
            "fs.MB.gas_inlet.pressure[*]": 2.50,
            "fs.MB.gas_inlet.temperature[*]": 293.0,

            "fs.MB.solid_inlet.flow_mass[*]": 549.0,
            "fs.MB.solid_inlet.temperature[*]": 1183.0,
            }
    inputs = fix_dynamic_inputs(m, time, dynamic_inputs)
    return inputs
...
def set_gas_values_to_inlets(m):
    ...
    inlet_names = [
        "fs.MB.gas_inlet.flow_mol[%s]" % t0,
        "fs.MB.gas_inlet.pressure[%s]" % t0,
        "fs.MB.gas_inlet.temperature[%s]" % t0,
        "fs.MB.gas_inlet.mole_frac_comp[%s,CO2]" % t0,
        "fs.MB.gas_inlet.mole_frac_comp[%s,H2O]" % t0,
        "fs.MB.gas_inlet.mole_frac_comp[%s,CH4]" % t0,
        ]
...
def set_solid_values_to_inlets(m):
    ...
    inlet_names = [
        "fs.MB.solid_inlet.flow_mass[%s]" % t0,
        "fs.MB.solid_inlet.temperature[%s]" % t0,
        #"fs.MB.solid_inlet.particle_porosity[%s]" % t0,
        "fs.MB.solid_inlet.mass_frac_comp[%s,Fe2O3]" % t0,
        "fs.MB.solid_inlet.mass_frac_comp[%s,Fe3O4]" % t0,
        "fs.MB.solid_inlet.mass_frac_comp[%s,Al2O3]" % t0,
        ]
    ...
    var_names = [
        ...
        #"fs.MB.solid_phase.properties[*,*].particle_porosity",
...
def set_gas_temperature_to_solid_inlet(m, include_fixed=False):
    ...
    for t, x in time*space:
        ...
        if not var.fixed or include_fixed:
            var.set_value(m.fs.MB.solid_inlet.temperature[t].value)
