...
def heat_exchangers(m, num_segments, intercooler):
    ...
    m.U = Param(initialize=1000, mutable=True, doc="W/m2/K")
    m.U_cooler = Param(initialize=500, mutable=True)
    m.U_heater = Param(initialize=750, mutable=True)
    ...
    m.cp_cw = Param(initialize=4184, mutable=True)

    m.cw_cooler_con = Constraint(
        expr=m.Q_cooler == -m.cw_cooler * m.cp_cw * (35 - 20),
    ...
    if intercooler:
        ...
        m.cw_intercooler_con = Constraint(
            expr=m.absorber.Q_intercooler == m.cw_intercooler * m.cp_cw * (35 - 20),
            doc="Heat calculation on cooling water side",
        )
    ...
    m.cp_co2_integrated_variation = Param(initialize=1, mutable=True)

    m.cp_co2_integrated = Expression(
        expr= m.cp_co2_integrated_variation * ((
    ...
    m.cp_h2o_integrated_variation = Param(initialize=1, mutable=True)
    m.cp_h2o_integrated = Expression(
        expr= m.cp_h2o_integrated_variation * ((
    ...
    m.h_vap_ex3 = Param(initialize=40800, mutable=True)
    ...
    def q_ex3_hot(m):
        return -m.Q_ex3 == -m.afs.stripper.vapor_properties[
            ...
            "H2O"
        ] * m.cp_h2o_integrated - m.h_vap_ex3 * (
