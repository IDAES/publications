...
"""
Liquid phase properties for the stripper column and flash tank, mixture of CO2-H2O-PZ
"""
...
class PZParameterData(PhysicalParameterBlock):
    ...
    def build(self):
        ...
        self.deltaH_PZ = Param(
            mutable=True,
            initialize=38750,
            units=pyunits.J / pyunits.mol,
            doc="Heat of phase change for PZ",
        )
        ...
        self.Hf_flash = Param(
            self.component_list,
            mutable=True,
            initialize={"CO2": -393500, "H2O": -241820, "PZ": 25000},
        )
        ...
        self.h_flash_variation = Param(initialize=1, mutable=True)

        self.P_sat_CO2_variation = Param(initialize=1, mutable=True)
        self.P_sat_H2O_variation = Param(initialize=1, mutable=True)
        self.P_sat_PZ_variation = Param(initialize=1, mutable=True)

        self.Cp_int_variation_CO2 = Param(initialize=1, mutable=True)
        self.Cp_int_variation_H2O = Param(initialize=1, mutable=True)
        self.Cp_int_variation_PZ = Param(initialize=1, mutable=True)

        self.Habs_param_1 = Param(initialize=-11054, mutable=True)
        self.Habs_param_2 = Param(initialize=4958, mutable=True)
        self.Habs_param_3 = Param(initialize=10163, mutable=True)
        ...
        obj.define_custom_properties(
            {
                ...
                "Cp_int": {"method": "_integral_heat_capacity"},
                "h_flash": {"method": "_flash_enthalpy"},
            }
        )
...
class PZStateBlockData(StateBlockData):
    ...
    def _integral_heat_capacity(self):
        _add_integral_heat_capacity(self)

    def _flash_enthalpy(self):
        _add_flash_enthalpy(self)
...
def _add_flow_mol_comp(self):
    x_in = {"CO2": 0.05, "H2O": 0.87, "PZ": 0.08}
...
def _add_equilibrium_pressure(self):
    ...
    if self.config.defined_state is False:
        ...
        def equilibrium_pressure_rule(b, j):
            if j == "CO2":
                return self.equilibrium_pressure[j] / pyunits.Pa == self.params.equilibrium_pressure_CO2_variation * \
                       exp(
                    self.params.equilibrium_pressure_CO2_param_1
                    + self.params.equilibrium_pressure_CO2_param_2 / self.temperature
                    + self.params.equilibrium_pressure_CO2_param_3 * (self.alpha**2)
                    + self.params.equilibrium_pressure_CO2_param_4
                    * self.alpha
                    / self.temperature
                    + self.params.equilibrium_pressure_CO2_param_5
                    * (self.alpha**2)
                    / self.temperature
                )
            elif j == "PZ":
                return self.equilibrium_pressure[j] / pyunits.Pa == self.params.equilibrium_pressure_PZ_variation * \
                       self.mole_frac_comp[
                    j
                ] * exp(
                    self.params.equilibrium_pressure_param_1[j]
                    - self.params.equilibrium_pressure_param_2[j] / self.temperature
                    - self.params.equilibrium_pressure_param_3[j]
                    * log(self.temperature / pyunits.K)
                    + self.params.equilibrium_pressure_param_4[j]
                    * self.temperature**2
                )
            elif j == "H2O":
                return self.equilibrium_pressure[j] / pyunits.Pa == self.params.equilibrium_pressure_H2O_variation * \
                       self.mole_frac_comp[
                    j
                ] * exp(
                    self.params.equilibrium_pressure_param_1[j]
        ...
        self.equilibrium_pressure_eq = Constraint(
            self.component_list, rule=equilibrium_pressure_rule
        )
...
def _add_density(self):
    ...
    self.T_ref = Param(initialize=273.15, mutable=True, units=pyunits.K)
    ...
    if self.config.defined_state is False:
        ...
        def dens_H2O_rule(b):
            return (
                self.dens_mass_H2O
                == self.dens_mass_H2O_param_1
                - self.dens_mass_H2O_param_2 * (self.temperature - self.T_ref)
                - self.dens_mass_H2O_param_3 * (self.temperature - self.T_ref) ** 2
            )
    ...
    self.dens_mass_param_1 = Param(initialize=0.0407, units=pyunits.kg / pyunits.mole)
    self.dens_mass_param_2 = Param(initialize=0.008, units=pyunits.kg / pyunits.mole)
    self.dens_mass_param_3 = Param(initialize=0.991, units=pyunits.dimensionless)

    if self.config.defined_state is False:

        def dens_rule(b):
            return self.dens_mass == self.params.dens_mass_variation * \
                   self.dens_mass_H2O * (
                self.dens_mass_param_1
                * self.mole_frac_comp["CO2"]
                / self.mw_comp["CO2"]
                + self.dens_mass_param_2
                * self.mole_frac_comp["PZ"]
                / self.mw_comp["PZ"]
                + self.dens_mass_param_3
            )
...
def _add_rxn_rate_cst(self):
    self.reaction_rate = Var(
        ...
        doc="Reaction rate constant (m3/mol/s)",
    )
    ...
    self.reference_temperature = Param(initialize=298.15, mutable=True, units=pyunits.K)

    if self.config.defined_state is False:

        def rxn_rate_cst_rule(b):
            return self.reaction_rate == self.reference_rate * exp(
    ...
    if self.config.defined_state is False:
        ...
        def henry_cst_rule(b):
            return (
                self.henry
                == self.henry_param_1
                * exp(self.henry_param_2 / self.temperature)
                * 0.0001
            )
...
def _add_diffusivity_cst(self):
    ...
    self.diffusivity_param_1 = Param(
        initialize=0.024, mutable=True, units=pyunits.m**2 / pyunits.s
    )

    self.diffusivity_param_2 = Param(initialize=-2122, mutable=True, units=pyunits.K)

    if self.config.defined_state is False:

        def diffusivity_cst_rule(b):
            return self.diffusivity_cst == self.diffusivity_param_1 * exp(
                self.diffusivity_param_2 / self.temperature
            ) * (10 ** (-4)) * ((self.viscosity_water / self.visc_d) ** 0.8)
...
def _add_heat_of_absorption(self):
    ...
    if self.config.defined_state is False:
        ...
        def heat_absorption_rule(b):
            return b.deltaH_abs == - b.params.heat_of_absorption_variation * (
                round(value(const.gas_constant), 3)
                * pyunits.J
                / pyunits.mole
                / pyunits.K
            ) * (b.params.Habs_param_1 + b.params.Habs_param_2 * b.alpha +
                 b.params.Habs_param_3 * (b.alpha**2))
    ...
    if self.config.defined_state is False:
        ...
        def equilibrium_cst_rule(b, j):
            if j == 1 or j == 2 or j == 3:
                return self.log_k_eq[j] == self.param_A[j] + self.param_B[
                    j
                ...
                return (
                    self.log_k_eq[j]
                    == self.param_A[j] + self.param_B[j] / self.temperature
                )
...
def _add_saturation_pressure(self):
    self.P_sat = Var(
        self.component_list, initialize=500000, bounds=(0, None), units=pyunits.Pa
    )
    ...
    def saturation_pressure_rule(b, j):
        if j == "CO2":
            return (
                self.P_sat[j]
                == self.params.P_sat_CO2_variation * exp(
                    self.params.equilibrium_pressure_CO2_param_1
                    + self.params.equilibrium_pressure_CO2_param_2 / self.temperature
                    + self.params.equilibrium_pressure_CO2_param_3 * (self.alpha**2)
                    + self.params.equilibrium_pressure_CO2_param_4
            ...
            return self.P_sat[j] / pyunits.Pa == self.params.P_sat_H2O_variation  * \
                   exp(
                self.params.equilibrium_pressure_param_1[j]
                - self.params.equilibrium_pressure_param_2[j] / self.temperature
                - self.params.equilibrium_pressure_param_3[j]
                * log(self.temperature / pyunits.K)
                + self.params.equilibrium_pressure_param_4[j] * self.temperature**2
            )
        elif j == "PZ":
            return self.P_sat[j] / pyunits.Pa == self.params.P_sat_PZ_variation  * \
                   exp(
                self.params.equilibrium_pressure_param_1[j]
                - self.params.equilibrium_pressure_param_2[j] / self.temperature
                - self.params.equilibrium_pressure_param_3[j]
    ...
    self.saturation_pressure_eq = Constraint(
        self.component_list, rule=saturation_pressure_rule
    )


def _add_integral_heat_capacity(self):
    self.Cp_int = Var(self.component_list, initialize=4000, doc="J/mol/K")
    self.T_ref = Param(initialize=298)

    def integrated_heat_capacities_rule(f, j):
        if j == "CO2":
            return f.Cp_int[j] == f.params.Cp_int_variation_CO2 * ((
                18.86 * f.temperature
                + 0.07937 * (f.temperature**2) / 2
                - 6.7834 * 10 ** (-5) * (f.temperature**3) / 3
                + 2.4426 * 10 ** (-8) * (f.temperature**4) / 4
            ) - (
                18.86 * f.T_ref
                + 0.07937 * (f.T_ref**2) / 2
                - 6.7834 * 10 ** (-5) * (f.T_ref**3) / 3
                + 2.4426 * 10 ** (-8) * (f.T_ref**4) / 4
            ))
        elif j == "H2O":
            return f.Cp_int[j] == f.params.Cp_int_variation_H2O * ((
                33.80 * f.temperature
                - 0.00795 * (f.temperature**2) / 2
                + 2.8228 * 10 ** (-5) * (f.temperature**3) / 3
                - 1.3115 * 10 ** (-8) * (f.temperature**4) / 4
            ) - (
                33.80 * f.T_ref
                - 0.00795 * (f.T_ref**2) / 2
                + 2.8228 * 10 ** (-5) * (f.T_ref**3) / 3
                - 1.3115 * 10 ** (-8) * (f.T_ref**4) / 4
            ))
        else:
            return f.Cp_int[j] == f.params.Cp_int_variation_PZ * 237 * (f.temperature - 413)

    self.integrated_heat_capacities_eq = Constraint(
        self.component_list, rule=integrated_heat_capacities_rule
    )


def _add_flash_enthalpy(self):
    self.h_flash = Var()

    def h_rule(f):
        return f.h_flash == f.params.h_flash_variation * sum(
            f.mole_frac_comp[j] * (f.params.Hf_flash[j] + f.Cp_int[j])
            for j in f.component_list
        )

    self.h_flash_eq = Constraint(rule=h_rule)
