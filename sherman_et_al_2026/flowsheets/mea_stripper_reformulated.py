...
class MEAStripperFlowsheetData(FlowsheetBlockData):
    ...
    def print_column_design_parameters(self):
        ...
        outlet = self.reboiler.bottoms
        print("Reboiler outlet CO2 Loading: ", pyo.value(outlet.mole_frac_comp[0, "CO2"]/outlet.mole_frac_comp[0, "MEA"]))
        print("Reboiler outlet H2O Loading: ", pyo.value(outlet.mole_frac_comp[0, "H2O"]/outlet.mole_frac_comp[0, "MEA"]))
    ...
    def strip_statevar_bounds_for_stripper_initialization(self):
        ...
        for t in self.time:
            for props in [
                ...
                self.reflux_mixer.reflux_state[t],
                self.reflux_mixer.rich_solvent_state[t],
            ]:
    ...
    def add_condenser_reboiler_performance_equations(self):
        ...
        COND.cooling_water_temperature_inlet = pyo.Param(
            initialize=16 + 273.15, # 16 deg C, converted to K
            mutable=True, # Pyomo wants it to be mutable if it has units
            units=pyo.units.K
        ) 
        COND.cooling_water_temperature_outlet = pyo.Var(
            self.time,
            initialize=27 + 273.15, # 27 deg C, converted to K
            bounds=(0, None),
            units=pyo.units.K
        ) 
        COND.cooling_water_temperature_outlet.fix()
        
        COND.DeltaT_hot_end = pyo.Var(
            self.time,
            initialize=10,
            units=pyo.units.K,
            bounds=(0, None),
            doc="intermediate variable of temp diff",
        )
        @COND.Constraint(self.time)
        def DeltaT_hot_end_eqn(b, t):
            return b.DeltaT_hot_end[t] == (
                b.liquid_phase[t].temperature 
                - b.cooling_water_temperature_outlet[t]
            )
        
        for t in self.time:
            calculate_variable_from_constraint(
                COND.DeltaT_hot_end[t], COND.DeltaT_hot_end_eqn[t])
        
        COND.DeltaT_cold_end = pyo.Var(
            self.time,
            initialize=10,
            units=pyo.units.K,
            bounds=(0, None),
        ...
        @COND.Constraint(self.time)
        def DeltaT_cold_end_eqn(b, t):
            return b.DeltaT_cold_end[t] == (
                b.liquid_phase[t].temperature 
                - b.cooling_water_temperature_inlet
            )

        for t in self.time:
            calculate_variable_from_constraint(
                COND.DeltaT_cold_end[t], COND.DeltaT_cold_end_eqn[t])
        
            
        # Define condenser (COND) LMTD variable and constraint
        COND.LMTD = pyo.Var(
            self.time,
            initialize=1,
            units=pyo.units.K,
            bounds=(0, None),
            doc="log mean temp diff",
        )
        @COND.Constraint(self.time)
        def LMTD_eqn(b, t):
            return b.LMTD[t] == (
                ...
                pyo.log(
                   b.DeltaT_hot_end[t] / b.DeltaT_cold_end[t]
                )
            )           
        
        for t in self.time:
            calculate_variable_from_constraint(
                COND.LMTD[t], COND.LMTD_eqn[t])
        ...
        def heat_transfer_eqn(b, t):
            # Negative sign on heat duty bc it's defined for the stream heat is being removed from
            return b.area * b.overall_heat_transfer_coefficient * b.LMTD[t]  == -b.heat_duty[t]
        
        calculate_variable_from_constraint(
            COND.area, COND.heat_transfer_eqn[0])

        # Condenser cooling agent flowrate
        COND.cooling_flow_mol = pyo.Var(
            self.time,
            initialize=1500,
            units=pyo.units.mol/pyo.units.s,
            doc="Condenser cooling agent molar flowrate",
            )
        
        COND.Cp_water = pyo.Param(
            initialize=75.3, # Value from Wolfram Alpha
            mutable=True, 
            units=pyo.units.J/(pyo.units.mol*pyo.units.K)
        )
        @COND.Constraint(self.time)
        def cold_side_energy_balance_eqn(b, t):
            return b.heat_duty[t] == (
                b.cooling_flow_mol[t]
                * b.Cp_water
                * (
                    b.cooling_water_temperature_inlet
                    - b.cooling_water_temperature_outlet[t]
        ...
        for t in self.time:
            calculate_variable_from_constraint(
                COND.cooling_flow_mol[t], COND.cold_side_energy_balance_eqn[t])

        iscale.set_scaling_factor(COND.area, 1e-3)
        
        for t in self.time:
            iscale.set_and_get_scaling_factor(COND.cooling_flow_mol[t], 1e-5)
            sT = iscale.get_scaling_factor(COND.liquid_phase[t].temperature )
            iscale.set_scaling_factor(COND.cooling_water_temperature_outlet[t], sT)
            iscale.constraint_scaling_transform(
                COND.DeltaT_cold_end_eqn[t], sT
            )
            iscale.constraint_scaling_transform(
                COND.DeltaT_hot_end_eqn[t], sT
            )
            iscale.constraint_scaling_transform(
                COND.LMTD_eqn[t], sT
            )
            iscale.constraint_scaling_transform(
                COND.cold_side_energy_balance_eqn[t], 1e-8
            )
            iscale.constraint_scaling_transform(
                COND.heat_transfer_eqn[t], 1e-8
            )

        ## Stripper reboiler area calculation, using formulas from the costing module
        REB = self.reboiler
        
        REB.overall_heat_transfer_coefficient = pyo.Param(
            initialize=1500, # Geankoplis (2011) p. 300 gives U = 1420-2270 W/m^2.K for steam/boiling water
            mutable=False,
            units=pyo.units.W/pyo.units.m**2/pyo.units.K
        )
        ...
        REB.steam_temperature_inlet = pyo.Param(
            initialize=308 + 273.15, mutable=False, units=pyo.units.K) # 308 deg C, converted to K
        REB.condensate_temperature_outlet = pyo.Var(
            self.time,
            initialize=151 + 273.15,
            units=pyo.units.K
        ) # 151 deg C, converted to K
        # Upper bound is 151 C bc that's the temperature it's being provided at
        # Lower bound is ~105 C (boiling point of water at 1.2 bar) bc we don't
        # want to have to pull a vacuum
        REB.condensate_temperature_outlet.fix()

        REB.Cp_steam = pyo.Param(
            # Computed with IAPWS95 at enth_mol=49122.4 J/mol
            # (approximately 125 C) and pressure= 1e5 Pa
            initialize=36.2,
            units=pyo.units.J / (pyo.units.mol * pyo.units.K),
            mutable=True
        )
        REB.latent_heat_steam = pyo.Param(
            initialize=38079.7, # Evaluated with IAPWS95 at 150 C
            units=pyo.units.J / pyo.units.mol,
            mutable=True
        )

        @REB.Expression(self.time)
        def steam_heat_per_mol(b, t):
            return (
                b.Cp_steam * (b.steam_temperature_inlet - b.condensate_temperature_outlet[t]) # Sensible heat
                + REB.latent_heat_steam # Latent heat
            )
        REB.steam_flow_mol = pyo.Var(
            self.time,
            initialize=100,
            units=pyo.units.mol/pyo.units.s
        )

        @REB.Constraint(self.time)
        def utility_energy_balance_eqn(b, t):
            return b.heat_duty[t] == b.steam_heat_per_mol[t] * b.steam_flow_mol[t]
        
        for t in self.time:
            iscale.constraint_scaling_transform(
                REB.utility_energy_balance_eqn[t], 1e-8
            )
        ...
        def heat_transfer_eqn(b, t):
            return b.heat_duty[t] == (
                ...
                * b.area
                * (
                    b.condensate_temperature_outlet[t]
                - b.vapor_reboil.temperature[t]
                )
            )
        iscale.set_scaling_factor(REB.area, 1e-3)
        
        for t in self.time:
            iscale.set_and_get_scaling_factor(REB.steam_flow_mol[t], 1e-4)
            sT = iscale.get_scaling_factor(REB.vapor_reboil.temperature[t])
            iscale.set_scaling_factor(REB.condensate_temperature_outlet[t], sT)
            iscale.constraint_scaling_transform(
                REB.heat_transfer_eqn[t], 1e-8
            )

        for t in self.time:
            calculate_variable_from_constraint(REB.steam_flow_mol[t], REB.utility_energy_balance_eqn[t])
        calculate_variable_from_constraint(REB.area, REB.heat_transfer_eqn[0])
