...
function show_help() {
    ...
    echo "  -t  [Optional] CO2 capture targets of interest (wrapped in quotations marks)"
    echo "      (default: '$co2_capture_targets')"
    echo "  -l  [Optional] PyROS confidence levels of interest (wrapped in quotation marks)"
    echo "      (default: '$pyros_conf_lvls')"
    echo "  -p  [Optional] True to include postsolve robust feasibility assessment"
    echo "      (a.k.a. 'heatmap') runs in studies, False otherwise"
    echo "      (default: $include_heatmaps)"
