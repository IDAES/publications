...
def run_parameter_sweep(
    ...
    if ipopt_options is None:
        ...
        # If an option was provided, use it, otherwise use
        # the default.
    ...
    for i, (flow, conv) in enumerate(itertools.product(
        ...
        setpoint = sp_data
        solve = SolveData(
            # TODO: Change the data type of this "status" field to something
            # consistent between SolverFactory and CyIpopt solvers.
            #
            # String is good because then this is json-serializable
            str(solve_data.status.solver.termination_condition),
            solve_data.values,
            solve_data.time,
        )
...
if __name__ == "__main__":
    ...
    print("Full space parameter sweep")
    print(
    ...
    n_total = len(results)
    converged_results = [
        res for res in results if res.solve.status == "optimal"
    ]
    n_converged = len(converged_results)
    print("\nConverged %s/%s problems" % (n_converged, n_total))

    with open("full_space.json", "w") as fp:
